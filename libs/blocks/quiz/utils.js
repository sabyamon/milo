...
export const handleResultFlow = async (answers = []) => {
  ...
  const redirectUrl = getRedirectUrl(primaryProducts, answers);
  ...
  allcards.forEach((selection) => {
    ...
    getAllSelectedQuestionsRelatedOptions.forEach(( { options, next }) => {
      if (options === selection) {
        ...
        const filteredNextSteps = flowStepsList.filter((val) => val != 'RESULT');
...
export const transformToFlowData = (questionData, userSelection) => {
